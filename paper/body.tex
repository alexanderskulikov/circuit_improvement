...
\todo[inline]{Nikita, please paste the code here}
...
\foreach \n/\x/\y in {1/0/4, 2/1/4, 3/2/4}
  \node[input] (x\n) at (\x, \y) {$x_{\n}$};
\node[gate,label=left:$a$] (g1) at (1,3) {$\oplus$};
\node[gate,label=right:$w_0$] (g2) at (2,3) {$\oplus$};
\node[gate, label=left:$b$] (g3) at (0,2) {$\oplus$};
\node[gate,label=left:$c$] (g4) at (1,1.5) {$\lor$};
...
\foreach \n/\x/\y in {1/0/5, 2/1/5, 3/2/5, 4/3/5}
  \node[input] (x\n) at (\x, \y) {$x_{\n}$};
\node[gate,label=left:$a$] (g1) at (1,4) {$\oplus$};
\node[gate,label={[shift={(-0.2,0.01)}]$b$}] (g2) at (2,4) {$\oplus$};
\node[gate,label=right:$w_0$] (g3) at (3,4) {$\oplus$};

\node[gate, label=left:$c$] (g4) at (0.5,3) {$\oplus$};
\node[gate,label=left:$d$] (g5) at (3,3) {$>$};

\node[gate,label=left:$e$] (g6) at (1,2) {$\lor$};
\node[gate,label={[shift={(0.2,0.01)}]$f$}] (g7) at (2,1.5) {$\oplus$};
...
\node[gate,label=left:$a$] (g1) at (1,4) {$\oplus$};
\node[gate,label={[shift={(-0.2,0.01)}]$b$}] (g2) at (2,4) {$\oplus$};
\node[gate,label={[shift={(-0.2,0.01)}]$c$}] (g3) at (3,4) {$\oplus$};
\node[gate,label=right:$w_0$] (g4) at (4,4) {$\oplus$};

\node[gate,label=left:$d$] (g5) at (0.5,3) {$\oplus$};
\node[gate,label=right:$e$] (g6) at (4,3) {$\oplus$};

\node[gate,label=left:$f$] (g7) at (1,2) {$\lor$};
\node[gate,label=right:$g$] (g8) at (3.5,2) {$>$};

\node[gate,label=left:$h$] (g9) at (2,1) {$\oplus$};
...
\todo[inline]{Nikita, please add forbidding wires to this code and ensure that it produces the right result}
...
Our program is~not able to~find out that the circuit of size~$31$ is~suboptimal. It~would be~interesting to~find
