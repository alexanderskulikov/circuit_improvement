...
\todo[inline]{Aleksander, CircuitFinder instead of find circuit}
...
a~half iterations of~the bubble sort algorithm (where one sorts two bits $x,y$ as~follows: $\min\{x,y\}=x\lor y, \max\{x,y\} = x \land y$). For $n=12$, the corresponding circuit of size~$31$ is~shown in~Figure~\ref{figure:thr31}.
...
\node[gate, label=right:$w_0$] (g31) at (9.3,9) {$\lor$};
...
\node[input] (x1) at (0, 4) {$x_{1}$};
\foreach \n/\x/\y in {2/1.5/5, 3/3/5, 4/4.5/5, 11/7.5/5, 12/9/5}
  \node[input] (x\n) at (\x, \y) {$x_{\n}$};

\foreach \x/\y/\z in {1/2/1.5, 2.5/3.5/3, 4/5/4.5, 7/8/7.5} {
  \draw (\x,3.8) rectangle (\y,4.2);
  \node at (\z,4) {SORT};
}

\node[gate] (g1) at (9,4) {$\land$};
\node[gate] (g2) at (5,2) {$\lor$};
...
\end{mypic}
\caption{$\size(\THR_{12}^2)=31$}
\label{figure:thr31_new}
\end{figure}


Dunne~\cite{Dunne84} came~up with
a~circuit of size~$2n+o(n)$ based on~the following clever trick. Organize the input bits into a~$\sqrt{n} \times \sqrt{n}$ matrix. Then, there are at least two~$1$'s among 
the input bits if and only if~there are two rows or~two
columns both containing~$1$. This observation allows to~proceed as~follows: compute the disjunction of all rows and columns (in~at most $2n$~gates), compute $\THR_{\sqrt{n}}^2$ for all rows and columns, and return
the disjunction of the results. (Sergeev~\cite{Sergeev2020} showed recently that the \emph{monotone} circuit size of $\THR_n^2$ is $2n+\Theta(\sqrt n)$.) For $n=12$, this 
leads to~a~circuit of size~$29$ shown in~Figure~\ref{figure:thr29}. 
...
\node[gate, label=left:$r_1$] (g10) at (1,5) {$\lor$};
\node[gate] (g6) at (2,5) {$\lor$};
\node[gate, label={[shift={(0.4,-0.8)}]$r_2$}] (g12) at (3,5) {$\lor$};
...
\node[gate, label={[shift={(0.08,-0.06)}]$r_3$}] (g14) at (4.5,4) {$\lor$};
\node[input] (x12) at (5.5, 4) {$x_{12}$};
\node[gate, label={[shift={(-0.1,-0.06)}]$c_2$}] (g5) at (6.5,4) {$\lor$};
\node[gate] (g15) at (7.5,4) {$\lor$};
\node[gate, label=right:$c_1$] (g2) at (8.5,4) {$\lor$};


\node[gate] (g24) at (2.5,3) {$\land$};
\node[gate] (g23) at (3.5,3) {$\lor$};
\node[gate, label={[shift={(-0.1,-0.06)}]$c_3$}] (g8) at (4.5,3) {$\lor$};
\node[gate, label={[shift={(0.3,-0.06)}]$r_4$}] (g16) at (5.5,3) {$\lor$};
