...
from generate_circuit.generate_ex3_circuit import *
from generate_circuit.generate_ex2_circuit import *
from generate_circuit.generate_sum_circuit import *
from generate_circuit.generate_ib_circuit import *
from generate_circuit.generate_maj_circuit import *
from generate_circuit.generate_mod3_circuit import *
from generate_circuit.generate_th_circuit import *
...
def shuffle(circuit, subcircuit_size=5, connected=True):
    for i in range(0, 100):
        shift = i * 20 + 10
        print(i)
        circuit = shuffle_circuit(circuit, subcircuit_size, connected, shift)
    circuit.save_to_file('shuffle_sum7')


def shuffle_circuit(circuit, subcircuit_size=5, connected=True, shift=0):
    ...
    allcombinations = list(combinations(circuit.gates, subcircuit_size))

    while True:
        subcircuit = allcombinations[random.randint(0, len(allcombinations))-1]
        if connected:
    ...
    improved_circuit = find_circuit(subcircuit_inputs, subcircuit_size, sub_in_tt, sub_out_tt, shift)
