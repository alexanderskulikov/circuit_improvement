...
class MOD3XorsCircuitFinder:
    def __init__(self, dimension, number_of_gates, xors_upper_bound, input_labels=None,
        ...
        self.xors_upper_bound = xors_upper_bound
    ...
    def init_default_cnf_formula(self):
        ...
        is_xors = [self.is_xor_variable(gate) for gate in self.internal_gates]
        for qq in combinations(is_xors, self.xors_upper_bound + 1):
            clause = [-lit for lit in qq]
            self.clauses.append(clause)
...
if __name__ == '__main__':
    for n, r in product(range(4, 5), range(3)):
        xors = 2 * n - 5
        print(f'n={n}, r={r}, xors={xors}')
