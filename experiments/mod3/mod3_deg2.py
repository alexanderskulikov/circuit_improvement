# checking the complexity of functions of the form [x1-x2+x1x2+x3+x4+... = 0 mod3]
...
def f(x):
    assert len(x) == 4
    # return [1, ] if x[0] - x[1] + x[0] * x[1] + x[2] + x[3] % 3 == 0 else [0, ]
    # return [1, ] if x[0] - ((x[0] + x[1]) % 2) + x[2] + x[3] % 3 == 0 else [0, ]
    # return [1, ] if (x[0]+x[1]+x[2]+2*x[3]+2*x[0]+x[2]+x[0]*x[3]+2*x[1]*x[2]+x[1]*x[3]+2*x[2]*x[3]) % 3 == 0 else [0, ]
    return [1, ] if (x[0] + x[1] + x[2] + x[3] + 2 * x[0] * x[2]) % 3 == 0 else [0, ]

# for size in range(7, -1, -1):
#     finder = MOD3XorsCircuitFinder(dimension=4, number_of_gates=size, function=f)
...
for c01, c02, c03, c12, c13, c23 in product(range(3), repeat=6):
    def g(x):
        return [1, ] if (x[0] + x[1] + x[2] + x[3] +
                         c01 * x[0] * x[1] +
                         c02 * x[0] * x[2] +
                         c03 * x[0] * x[3] +
                         c12 * x[1] * x[2] +
                         c13 * x[1] * x[3] +
                         c23 * x[2] * x[3]) % 3 == 0 else [0, ]

    for size in range(6, -1, -1):
        finder = MOD3XorsCircuitFinder(dimension=4, number_of_gates=size, function=g)
        circuit = finder.solve_cnf_formula(verbose=False)
        if not circuit:
            break

    print(c01, c02, c03, c12, c13, c23, size)